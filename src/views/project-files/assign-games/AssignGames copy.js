...
export default function AssignGames() {
    ...
    const [selectedGameIds, setSelectedGameIds] = useState([]);  // Global Selected Games Ids
 ...
 */
    ...
    const GameListDetails = (
        ...
        let params = JSON.stringify({
            ...
            // "client_account_ids": client_account_ids?.length > 0 ? client_account_ids : state.location_account_ids,
            status: "active",
        ...
        <div>
            ...
            <div className='card shadow-lg rounded mb-3' >
                <div className='card-body '>
                    ...
                    <div>
                        <span style={{ color: '#3d99f5' }}>ASSIGN OR UN-ASSIGN MULTIPLE GAMES TO OR FROM CLIENTS</span> <br />
                        <span style={{ color: '#db5d5d94' }}>
                            1. Select Client &nbsp;&nbsp;2. Select Client Account &nbsp;&nbsp; 3. Checkbox next to Game(s) to be Assigned OR Un-assigned &nbsp;&nbsp;4. Click Assign(BLUE) / Un-assign(RED) on the left &nbsp;&nbsp;
                ...
                <div className='col-sm-4' >
                    <div className='card shadow-lg rounded mb-3'  >
                        <div className='card-body' style={{ margin: 10 }}>
                            <div className="row ">
                                <button type="button"
                                    className="btn btn-sm btn-primary mb-2" onClick={() => IsConfirmed('assign')}
                                    disabled={
                                ...
                                >
                                    ASSIGN GAMES TO CLIENTS</button>
                                <button type="button" className="btn btn-danger btn-sm mb-3"
                                    onClick={() => IsConfirmed('deassign')} style={{ color: '#fff' }}
                                    disabled={
                                        state.selected_account_data.length > 0 &&
                                            state.selected_game_data.length > 0
                                            ? false
                                    ...
                                    UN-ASSIGN GAMES FROM SELECTED CLIENTS</button>
                            </div>
                        ...
                        <div className='card-body'>
                            ...
                            <div className="row">
                                ...
                                <div className="col-xxl-6 col-xl-3 col-lg-3 col-md-4 d-none d-md-block mt-4">
                                    <CInputGroup className="mb-3">
                                        ...
                                        <CFormInput type="text" autoComplete="off" placeholder="Search Games by Game ID" id="search" value={state.search} onChange={(e) => {
                                            e.preventDefault();
                                            setState((prevState) => ({
                            ...
                            <div style={{ margin: 10, overflowX: 'auto' }} className='table-responsive tableFixHead'>
                                <table className="table table-hover">
                                    <thead className="table-primary">
                                        <tr>
                                            {(state?.game_list && state?.game_list.length > 0) && <th><input className="form-check-input" type="checkbox" checked={state.isActive} id="flexCheckChecked" onChange={selectedGame} /></th>}
                                    ...
                                    <tbody>
                                        {state?.game_list && state?.game_list.length > 0 ? state?.game_list.map((data, key) => (
                                            <tr key={key} className="table-data">
                                                <td><input
                                                    type="checkbox"
                                                    className="form-check-input"
                                                    checked={data.isActive}
                                                    onChange={(ev) => onChangeSelectForGame(ev.target.checked, key,)}
                                                    style={{ marginRight: 5, borderColor: '#8fbc8f' }}
                                                /> </td>
                                                ...
                                                <td>
                                                    ...
                                                    <span style={{ textDecoration: 'none', color: state.copyText != data.game_id ? '' : '#1b9e3e' }}>{data.game_id.substr(0, 8) + '...'} </span>&nbsp;
                                                    <CopyToClipboard text={data.game_id} onCopy={() => onCopy(data.game_id)}>
                                        ...
                                        )) : <tr style={{ textAlign: 'center' }}><td colSpan="5">Games are not available.Please contact to admin.</td></tr>}
                                    </tbody>
                                </table>
                            </div>
                            <div className="row">
