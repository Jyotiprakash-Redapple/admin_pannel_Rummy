...
export default function WalletTransactionReport() {
  ...
  */
    ...
    const WalletTransactionReport = () => {
        if (state.account_id) {
            ...
            Service.apiPostCallRequest(RouteURL.WalletTransactionReport, params, token)
                .then((res) => {
                    if (res.err === Constants.API_RESPONSE_STATUS_SUCCESS) {
                        setState((prevState) => ({
                            ...prevState,
                            wallet_report: res?.data?.data,
                            wallet_sum_report: res?.data?.extraData,
    ...
    const exportToExcel = () => {
        ...
        const WalletReport = state.wallet_report; // This already contains all loaded transactions from all pages

        // Format the data to include only the fields that are displayed in the table
        const formattedData = WalletReport.map((data, key) => ({
            "SL No": key + 1,
            "Transactions Id": data.transaction_id,
            "Amount": data.amount,
            "Ava. credit": data.available_balance,
            "Type": camelCase(data.transaction_type),
            "Date": moment(data.created_at).format("YYYY-MM-DD"),
            "Trans. From a/c.": data.from_account_name ? data.from_account_name : "-",
            "Trans.To a/c.": data.to_account_name ? data.to_account_name : "-",
            "Trans. From User": data.from_user_username ? data.from_user_username : "-",
            "Trans. To User": data.to_user_username ? data.to_user_username : "-",
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Wallet_Wise_Report");
    ...
    return (<div >
        ...
        <div className='card shadow-lg rounded' >
            <div className='card-body'>
                <h3 className="hed_txt pt-2">Wallet Transaction Report</h3>
                ...
                <div className='row p-3'>
                    <div className="col-sm-3 mt-4">
                        <CInputGroup className="mb-3">
                            ...
                            <CFormInput type="text" autoComplete="off" placeholder="Search Wallet Report" id="search" value={state.search} onChange={(e) => {
                                e.preventDefault();
                                setState((prevState) => ({
                ...
                <div>
                    ...
                    >
                        ...
                        >
                            <span variant="h6">Wallet Transaction Report Filter</span>
                        ...
                        <Box sx={{ padding: "16px", width: "800px" }}>
                            ...
                            <div className='col-md-12 mt-3'>
                                ...
                                <select className="form-select mb-1" id="transaction_type" name="transaction_type" onChange={(e) => {
                                    e.preventDefault();
                                    setState((prevState) => ({
                                        ...prevState, transaction_type: e.target.value,
                                    }));
                                }}
                ...
                <div style={{ margin: 10 }} className="tableFixHead">
                    <div className="table-responsive mt-2">
                        <table className="table table-hover ">
                            ...
                            <tbody>
                                {state.wallet_report?.length > 0 ? (
                                    state.wallet_report.map((data, key) => (
                                        <tr key={key}>
                                            <td scope="row">
                                                <CopyToClipboard
